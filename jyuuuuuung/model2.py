...
corrList = []
def corr(p): return p['TARGET'].corr(p['GHI'])
...
for i in range(0, 90):
    solar_data['GHI'] = solar_data['DNI'] * \
        math.cos(math.pi/180*i)+solar_data['DHI']
...
solar_data['GHI'] = solar_data['DNI'] * \
    math.cos(math.pi/180*63)+solar_data['DHI']
...
for i in range(len(solar_data)):
    solar_data.loc[i, ["DP"]] = eval_dewpoint(
        float(solar_data.loc[i, ["T"]]), solar_data.loc[i, ["RH"]])

solar_data = solar_data.reindex(columns=[
                                "Day", "Hour", "Minute", "GHI", "DHI", "DNI", "WS", "RH", "T", "DP", "TARGET"])
...
sns.heatmap(data=solar_data.corr(), annot=True,
...
train_x, test_x, train_y, test_y = train_test_split(
    input_array, output_array, test_size=78/1086, shuffle=False)
train_x, val_x, train_y, val_y = train_test_split(
    train_x, train_y, test_size=48*3/1008, shuffle=False)
...
for i, q in enumerate(np.arange(0.1, 1, 0.1)):
    ...
    # #   model.reset_states()
    model.fit(train_x, train_y, epochs=100, batch_size=n_batch, shuffle=False,
              validation_data=(val_x, val_y), callbacks=[callback], verbose=2)
    predictions = []
    for k in range(81):
        prediction = model.predict(np.array([test[k]]), batch_size=1)
        predictions.append(prediction)
    predictions = np.reshape(np.concatenate(
        np.array(predictions), axis=0), (81*96))
